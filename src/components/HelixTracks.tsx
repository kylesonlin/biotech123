// src/components/HelixTracks.tsx
...
const HelixTracks: React.FC<Props> = ({
  ...
  // helix
  cycles = 2.5,                      // 5 crossings
  phase = 4.5945616035,              // mid crossing ≈ 54.5%
  ...
  useEffect(() => {
    // 1) Nuke any legacy overlays/guides in the DOM (belt + suspenders)
    document.querySelectorAll(
      ".helix-guides, .helix-track, .guide-center, .guide-lane, svg[data-helix-guide]"
    ).forEach(el => el.remove());

    const layer = layerRef.current!;
    // 2) Wipe any leftover children (from HMR etc.)
    layer.innerHTML = "";
    layer.setAttribute("data-helix-version", "1.1.0");
    console.log("[HelixTracks 1.1.0] props", { gapVh, y0, y1, c1, c2, cycles, phase, ampMin, ampMax });
    ...
    for (let i = 0; i < total; i++) {
      ...
      // Kill any rogue CSS animations/transitions that might be defined globally
